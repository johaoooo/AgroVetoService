...
export default function HomePage({ onAddToCart, setIsCartOpen }) {
  ...
  return (
    <div className="space-y-16 pb-16 bg-[#f6f8fa]">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4 border-b border-slate-200 pb-4">
          ...
          <Link
            to="/poles"
            className="text-emerald-800 hover:text-emerald-900 font-bold text-sm cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {POLES.map((pole) => (
            <div
              ...
              className="p-6 rounded-3xl bg-[#fafbfc] border border-slate-200/90 hover:border-emerald-500 hover:shadow-lg transition-all cursor-pointer flex flex-col justify-between group"
            >
              <div>
                <span className="text-xs font-bold text-slate-400 block mb-2">Pôle 0{pole.id}</span>
      ...
      <section className="bg-[#edf2f6] py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4 border-b border-slate-300/80 pb-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredProducts.map((product) => (
              <div
                key={product.id}
                className="rounded-3xl bg-[#fafbfc] border border-slate-200/90 p-5 shadow-sm hover:shadow-lg transition-all flex flex-col justify-between"
              >
                <div className="aspect-16/11 rounded-2xl overflow-hidden bg-slate-200 mb-4 shadow-2xs">
                  <img 
                    src={product.image} 
                    alt={product.name} 
                ...
                <div>
                  ...
                  <h4 className="font-bold text-slate-900 text-sm mb-2 line-clamp-2">{product.name}</h4>
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-emerald-100/80 hover:bg-emerald-700 hover:text-white text-emerald-900 font-bold text-xs transition-all text-center cursor-pointer shadow-2xs"
                >
                  Ajouter au panier
                </button>
              </div>
